...
class TestPlan2D(Node):
    def __init__(self):
        super().__init__('test_plan_2d')
        ...
        # 2D移動コマンド送信
        self.publisher_ = self.create_publisher(String,'/client_api/plan_2d',1)

        # テストコマンド1回送信
        self.send_test_plan_2d()
        


    def send_test_plan_2d(self):
        test_json = {
            "ID" : {
                "robot_name": "robotA",
        ...
        self.get_logger().info('Sending test 2D Plan command: "%s"' % str(test_json))
        msg = String()
        msg.data = json.dumps(test_json)
        self.publisher_.publish(msg)
        self.get_logger().info('Test 2D Plan command sent.')
